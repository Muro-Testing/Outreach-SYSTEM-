...
export function App() {
    ...
    async function onStopRun() {
        ...
        setError("");
        try {
            const stopped = await api(`/api/runs/${run.id}/cancel`, { method: "POST" });
            setRun(stopped);
        }
        catch (err) {
